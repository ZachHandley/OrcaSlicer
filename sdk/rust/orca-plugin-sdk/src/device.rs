...
pub struct HostEmit {
    ctx: *mut c_void,
    emit_status: extern "C" fn(
        *mut c_void,
        state:   i32,
        code:    i32,
        message: *const core::ffi::c_char,
    ),
    emit_message: extern "C" fn(
        *mut c_void,
...
impl HostEmit {
    pub fn emit_status(&self, state: PrinterState, code: i32, message: Option<&str>) {
        let c_msg = message.map(|m| std::ffi::CString::new(m).ok()).flatten();
        let ptr = c_msg.as_ref()
                       .map(|c| c.as_ptr())
...
pub fn register<A: DeviceAgent>() -> Result<()> {
    ...
    extern "C" fn destroy<A: DeviceAgent>(instance: *mut c_void) {
        ...
        unsafe { drop(Box::from_raw(instance as *mut A)); }
    }
    extern "C" fn current_state_thunk<A: DeviceAgent>(instance: *mut c_void) -> i32 {
        if instance.is_null() { return PrinterState::Disconnected as i32; }
    ...
    extern "C" fn connect_thunk<A: DeviceAgent>(
        ...
        if instance.is_null() { return abi::ORCA_ERR_INVALID_ARGUMENT; }
        let a = unsafe { &*(instance as *const A) };
        ...
        match a.connect(to_str(device_id), to_str(host_or_ip), port,
                        to_str(username), to_str(password), use_tls != 0) {
            Ok(()) => abi::ORCA_OK,
            Err(e) => e.as_code(),
        }
    }
    extern "C" fn disconnect_thunk<A: DeviceAgent>(instance: *mut c_void) -> i32 {
        if instance.is_null() { return abi::ORCA_ERR_INVALID_ARGUMENT; }
        let a = unsafe { &*(instance as *const A) };
        a.disconnect().map(|_| abi::ORCA_OK).unwrap_or_else(|e| e.as_code())
    }
    extern "C" fn send_command_thunk<A: DeviceAgent>(
        ...
        a.send_command(bytes).map(|_| abi::ORCA_OK).unwrap_or_else(|e| e.as_code())
    }
    extern "C" fn start_print_thunk<A: DeviceAgent>(
        ...
        a.start_print(to_str(gcode_path), to_str(job_name), start_immediately != 0)
            .map(|_| abi::ORCA_OK).unwrap_or_else(|e| e.as_code())
    }
    extern "C" fn cancel_print_thunk<A: DeviceAgent>(instance: *mut c_void) -> i32 {
        if instance.is_null() { return abi::ORCA_ERR_INVALID_ARGUMENT; }
        let a = unsafe { &*(instance as *const A) };
        a.cancel_print().map(|_| abi::ORCA_OK).unwrap_or_else(|e| e.as_code())
    }

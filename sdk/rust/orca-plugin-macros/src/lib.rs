...
use quote::quote;
use syn::{
    parse_macro_input, punctuated::Punctuated, Expr, ExprArray, ExprLit, ItemStruct,
    Lit, Meta, Token,
};
...
fn parse_args(input: TokenStream) -> syn::Result<PluginArgs> {
    ...
    for m in metas {
        ...
        match key.as_str() {
            ...
            "permissions" => {
                args.permissions = extract_lit_str_array(&nv.value)
                    .ok_or_else(|| syn::Error::new_spanned(
                        &nv.value, "permissions must be an array of strings"))?;
            }
            other => {
                return Err(syn::Error::new_spanned(
                    &nv.path,

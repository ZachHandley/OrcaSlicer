...
bool SyncAmsInfoDialog::is_same_nozzle_diameters(NozzleType &tag_nozzle_type, float &nozzle_diameter)
{
    ...
    try {
        PresetBundle *preset_bundle        = ::orca::session().presets().raw_ptr();
        auto          opt_nozzle_diameters = preset_bundle->printers.get_edited_preset().config.option<ConfigOptionFloats>("nozzle_diameter"); // TODO(orca-types): manual migration — captured ConfigOption pointer reused downstream for nullptr check and get_at()

        const ConfigOptionEnumsGenericNullable *nozzle_type = preset_bundle->printers.get_edited_preset().config.option<ConfigOptionEnumsGenericNullable>("nozzle_type"); // TODO(orca-types): Nullable surface not wired yet
